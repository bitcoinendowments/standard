...
| A Taproot key path bypasses the script path | Nothing structural distinguishes them | §2.6 makes the deployment declare, per path, whether the threshold is script enforced or produced by aggregate signing. §2.7 requires a published derivation, reproducible by anyone, wherever the key path is disabled. §2.8 makes an enabled key path an ordinary path, so it competes for weakest route. §2.9 forbids naming a protocol that cannot express the threshold claimed, and §2.10 forbids presenting an aggregate threshold as consensus enforced. Reproduce the derivation yourself and compare it against the output key |
