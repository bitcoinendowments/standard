...
**A guardian goes silent.** A deployment tolerates n minus m unavailable guardians before spending halts, per spending path. §9.6 requires that replacement be rehearsed and its real duration published, because an untested replacement procedure is the most common way this scenario becomes permanent.

**A guardian key is compromised.** One key is insufficient at any conformant threshold. The compromise must be detected, disclosed, and the coins moved to a policy that excludes the key. §9.7 exists because a declaration of revocation changes nothing: an attacker holding a share keeps it until the coins move. Detection is the weak link and it is a governance property, not a chain enforced one.

**Guardians collude at the threshold.** Any m guardians can move funds against the mission, and Bitcoin validates the transaction as correct, because it is correct. Nothing in this design prevents it. §3.5 makes it harder to arrange, §3.10 requires the real number be stated out loud using the deployment's own m and n, and §10 is where a legal wrapper may create consequences afterward. Consequences afterward are not prevention.

**The administrator commits fraud.** Blocked from holding a share by §4.2, blocked from choosing beneficiaries outside fixed criteria by §4.4, and exposed by the publication requirements in §4.7 and §7.1. The residual path is a well documented fraudulent request that guardians approve without thinking, which is why §7.4 puts the decision on each guardian individually and forbids delegating it.

**The coordinator turns malicious.** It can present a transaction that does not match the published request. The defence is each guardian verifying independently rather than trusting the interface presenting it. §7.4 requires the independent decision and §7.5 requires the verification procedure be published, but this standard does not specify that procedure, which is why it belongs to BES 0002. This property degrades quietly as a deployment gets busy.

**Legal seizure or compulsion.** A guardian may be compelled in their own jurisdiction. §3.7 requires a documented coercion analysis with its residual uncertainty stated, rather than a claim that no jurisdiction can reach m, which cannot be proved. Counting countries does not satisfy it. Three countries where one actor can reach three guardians is worse than two countries where nobody can reach m.
...
**A fee spike prevents timely settlement.** Rehearsed under §9.3, and necessarily simulated rather than observed, since a test network cannot reproduce a mainnet fee market. §9.5 requires that a scenario be marked observed or simulated, so nobody later mistakes a rehearsal for evidence.

**An undisclosed spending path exists.** A three of five script path sits alongside a key path or recovery branch that spends with less. Every other control in this document is decoration if this is true. §2.1 requires the complete policy be published, §2.2 requires it reconstruct the actual on chain output, §2.3 requires the weakest condition be stated, and §2.4 forbids a weaker undisclosed path. §2.7 is the specific case worth naming: a Taproot key path left enabled and unmentioned spends with whatever the internal key can produce, which is why disabling it requires publishing the derivation that proves it. A verifier who checks nothing else should check this.

**A coordinator substitutes a receive address.** Donations flow to an address that does not belong to the endowment. §2.12 requires that addresses be derivable from the published policy and §2.13 requires each be verified against that derivation before publication.
...
Every statement here is parameterized by a deployment's own m and n, per §3.10, and by every spending path separately. Loss of n minus m guardians does not stop fund movement. Collusion among m can move funds against the mission. A rule engine can be bypassed by guardians signing a different valid transaction unless the restriction is committed in the spending condition. A legal wrapper depends on a court's cooperation. Detection of a compromised key depends on people paying attention, and until the coins move, a compromised key still works.
