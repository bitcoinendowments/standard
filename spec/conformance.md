...
2. Validate the manifest structure against the schema for that release, then run the cross field checks in `../tests/check_invariants.py`. Both check assertions, not conformance, under §11.6.
3. Derive the addresses from the published spending policy under §2.1, and reconstruct the on chain output under §2.2, including any Taproot output key and script tree commitment. This is where a distributed authority claim becomes checkable, or fails to.
4. Read the weakest authorization condition under §2.3 and satisfy yourself that no path in the published policy is weaker. A three of five script path means nothing if a key path or a recovery branch spends with one signature. Run `../tests/check_invariants.py`, which checks the deployment's own numbers against each other.
5. Check the m of n arithmetic under §3.10 against the actual policy, per path.
6. Read the domain tables under §3.3 and the coercion analysis under §3.7, including its stated residual uncertainty. The arithmetic is checkable. Whether the table names every real dependency is documentary, and it is where a false claim is most likely to survive.
